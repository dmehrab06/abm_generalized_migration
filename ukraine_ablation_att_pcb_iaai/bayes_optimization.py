...
import subprocess
subprocess_output = subprocess.run(["squeue","-u","zm8bh","--name=bayes_att_pbc"],capture_output=True)

print(len(str(subprocess_output)))

if len(str(subprocess_output))==230:
    ...
    if len(optimizer.space)>=0:
        ...
        for f in all_files[0:-3]:
            os.remove(f)
        hyper_comb = len(optimizer.space)+2000
