...
def calc_attitude_parallel(args):
    cur_household_data,lookahead_date_1,lookahead_date_2,min_date = args
...
def multiproc_attitude(cur_household_data, lookahead_date_1, lookahead_date_2, min_date):
    ...
    pool_args = [(h_chunk,lookahead_date_1,lookahead_date_2,min_date) for h_idx,h_chunk in enumerate(hh_splits)]
...
def multiproc_peer_effect(temp_household):
    ...
    pool_args = [h_chunk for h_idx,h_chunk in enumerate(hh_chunks)]
    pool = mp.Pool(processes = cpus)
...
neighbor_household_data = pd.read_csv(HOUSEHOLD_DIR+NEIGHBOR_DATA_PREFIX+PLACE_NAME+'_'+str(STRUCT)+'_s2.csv')
...
for i in range(0,300):
    ...
    if USE_CORE==1:
        ...
        home_conflict_df = multiproc_attitude(cur_household_data,lookahead_date_1,lookahead_date_2,min_date)
...
data = {'raion': [PLACE_NAME],'runtime':[end-start_time],'hyper_comb':[hyper_comb],
        'start_time':start,'end_time':end,'memory_consumed':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}
