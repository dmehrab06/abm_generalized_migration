...
def bernoulli_border_trap(val,moves,current_phase,trap_status,multiply_lo=0.8,multiply_hi=1.5):
    if trap_status==0:
        return bernoulli_border(val,moves,current_phase,multiply_lo,multiply_hi)
    else:
        if val<=0.001:
            if val<=0.0001:
                return 2
...
def propagate_prob_conflict(next_val,cur_val,fixed_point):
    if fixed_point==1:
        return next_val
    return cur_val
...
def refine_through_peer_effect(temp_household,current_phase=0):
    ...
    highest_fear = new_temp_households['prob_conflict'].max()
    new_temp_households['prob_conflict'] = new_temp_households.apply(lambda x: propagate_prob_conflict(highest_fear,x['prob_conflict'],x['fixed_point']),axis=1)
...
def calc_attitude(cur_impact_data,cur_household_data,min_date):
    ...
    if(cur_impact_data.shape[0]==0):
        impact_in_homes = cur_household_data.copy()
...
def calc_attitude_parallel(args):
    ...
    if(cur_impact_data.shape[0]==0):
        #print('letting me handle the thing')
        impact_in_homes = cur_household_data.copy()
        impact_in_homes['prob_conflict'] = impact_in_homes['prob_conflict'].apply(lambda x: memory_decay(x,S))
        cur_household_data = cur_household_data.drop(columns='prob_conflict')
        home_conflict_df = impact_in_homes.groupby(['hid'])['prob_conflict'].sum().reset_index()
...
def multiproc_attitude(cur_household_data, impact_data, lookahead_date_1, lookahead_date_2, min_date):
    ...
    cur_impact_data = impact_data[(impact_data.time>=lookahead_date_1) & (impact_data.time<=lookahead_date_2)]
    print('impact data',cur_impact_data.shape[0],flush=True)
...
if __name__ == "__main__":
    ...
    trapped_date_start = pd.to_datetime(str(sys.argv[22]))
    trapped_date_end = pd.to_datetime(str(sys.argv[23]))
    ...
    hh_samples = cur_household_data.sample(1000).hid.tolist()
...
#             simulated_leaving_df = simulated_leaving_df.append(new_row_2,ignore_index=True)
#             continue
        sample_household_actions = pd.DataFrame(columns=['time','Raion','hid','fear','intention','decision','active_neighbors'])
        ...
        trapped = 0
        if (min_date>=trapped_date_start) and (min_date<=trapped_date_end):
            temp_households['moves'] = 0
            trapped = 1
        ...
        temp_households['move_type'] = temp_households.apply(lambda x: bernoulli_border_trap(x['move_type_random'],x['moves'],phase,trapped,multiply_lo,multiply_hi),axis=1)
        ...
        if((curtime-start)>=1):
            ...
            start = curtime
        temp_households['move'] = temp_households['move_type']
        ...
        sampled_hhs_with_decision = temp_households[temp_households.hid.isin(hh_samples)][['hid','prob_conflict','h_lat','h_lng','moves','s2_cell']]
        sampled_hhs_with_decision = sampled_hhs_with_decision.rename(columns={'prob_conflict':'fear','moves':'decision'})
        sampled_hhs_with_decision['intention'] = sampled_hhs_with_decision['fear'].apply(lambda x: aggregated_prob_conflict(x))
        sampled_hhs_tracker_df = ((temp_households.groupby('s2_cell')['hid'].count().reset_index()).rename(columns={'hid':'neighbors'})).merge(sampled_hhs_with_decision,on='s2_cell',how='inner')
        sampled_hhs_tracker_df['time'] = str(min_date)
        household_action_tracker_df.append(sampled_hhs_tracker_df)
...
#         who_went_where.append(moving_households_df)
    ...
    ##################################6
    household_history_tracker_df = pd.concat(household_action_tracker_df)
    household_history_tracker_df.to_csv(OUTPUT_DIR+'hh_action_history_'+str(PLACE_NAME)+'_'+str(hyper_comb).zfill(5)+'.csv',index=False)
