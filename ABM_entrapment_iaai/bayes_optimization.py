...
import subprocess
subprocess_output = subprocess.run(["squeue","-u","zm8bh","--name=abm"],capture_output=True)

print(len(str(subprocess_output)))

if len(str(subprocess_output))==220:
    ...
    if len(optimizer.space)>=0:
        ...
        for f in all_files[0:-3]:
            os.remove(f)
        hyper_comb = len(optimizer.space)+6000
    ...
    ews = X['ews']
    pactive = 40
    peer_thresh_lo = X['p_hi']
    peer_thresh_hi = X['p_lo']
    border_cross_prob = X['b_prob']
    phase_shift = 40
    multiply_lo = 1.0
    multiply_hi = 1.8
    trap_start_date = '2022-01-01'
    trap_end_date = '2022-01-01'
    comment = 'after_fixing_no_impact_and_trap_calibration_'+str(hyper_comb)
    ...
    for i in range(len(set1_raion)):
        ...
        if name.startswith('Chornobyl'):
            print('"'+name+'"',hyper_comb,D,A,T,S,t_l,t_r,ps,ews,pactive,peer_thresh_lo,peer_thresh_hi,
                  network_struct,dynamic_network,use_neighbor,border_cross_prob,phase_shift,multiply_lo,multiply_hi,core_use,trap_start_date,trap_end_date,file=f)
        else:
            print(name,hyper_comb,D,A,T,S,t_l,t_r,ps,ews,pactive,peer_thresh_lo,peer_thresh_hi,
                  network_struct,dynamic_network,use_neighbor,border_cross_prob,phase_shift,multiply_lo,multiply_hi,core_use,trap_start_date,trap_end_date,file=f)
    ...
    for i in range(len(set2_raion)):
        ...
        if name.startswith('Chornobyl'):
            print('"'+name+'"',hyper_comb,D,A,T,S,t_l,t_r,ps,ews,pactive,peer_thresh_lo,peer_thresh_hi,
                  network_struct,dynamic_network,use_neighbor,border_cross_prob,phase_shift,multiply_lo,multiply_hi,core_use,trap_start_date,trap_end_date,trap_start_date,trap_end_date,file=f)
        else:
            print(name,hyper_comb,D,A,T,S,t_l,t_r,ps,ews,pactive,peer_thresh_lo,peer_thresh_hi,
                  network_struct,dynamic_network,use_neighbor,border_cross_prob,phase_shift,multiply_lo,multiply_hi,core_use,trap_start_date,trap_end_date,file=f)
